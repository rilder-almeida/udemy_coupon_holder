...
class Scraper:
    ...
    def _get_course_id(self):
        try:
            # click(Button(text='Comprar agora', below='Adicionar ao carrinho'))
            # click(Button(text='Comprar agora'))
            tik().s
            scroll_down(rand_scroll(5))
            tik().s
            click('Comprar Agora')
        except Exception:
    ...
    def get_course(self):
        ...
        pre_index = 0
        for part in result:
            if part.startswith('Classificação:'):
                clas_index = result.index(part)
            if part.startswith('Pré-visualizar'):
                pre_index = result.index('Pré-visualizar este curso')
    ...
    def get_coupon(self):
        ...
        if Button(text='Inscreva-se agora').exists:
            return {
                'coupon_url': coupon_url,
                'coupon_key': self.processed_url['coupon_key'],
                'is_valid': True
